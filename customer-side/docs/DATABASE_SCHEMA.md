...
* **host_id:** String *(NEW — the user_id of the session creator/host)*
...
> This sub-collection powers the Host/Join approval flow. Each document represents one person's request to join the session.
...
## `online_orders` Collection *(NEW — Staff-Side Managed)*
> This collection is written to by the staff-side aggregator integration module. The customer-side does NOT interact with this collection directly, but it is documented here for shared schema awareness.
...
* **items:** Array of Maps (`{ name, quantity, price, modifiers }`)
