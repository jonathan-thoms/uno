...
* **Session Initialization (Host/Join Model):** QR code encodes the `table_id`. When the first person scans the QR code for a table with no active session, they become the **Host** of a new session. They are prompted to enter a **nickname** before the session is created. The session document stores the host's user ID in a `host_id` field.
...
### Module E: Online Ordering Aggregator Integration (Staff-Side)
* **Aggregator Dashboard:** The staff-side interface integrates with third-party online ordering platforms (**Swiggy**, **Zomato**, and potentially others) to receive and manage delivery/pickup orders alongside dine-in orders.
* **Unified Order Queue:** Online orders from aggregators appear in the same Cashier/Kitchen interface as dine-in QR orders, clearly tagged with their source platform (e.g., a "Swiggy" badge or "Zomato" badge) for easy identification.
* **Order Lifecycle:** Each aggregator order follows a lifecycle: `received` → `preparing` → `ready` → `dispatched` → `completed`. Staff update the status from the POS interface.
* **Separate Collection:** Aggregator orders are stored in an `online_orders` Firestore collection (not the `sessions` collection), keeping dine-in and delivery data cleanly separated while visible in the same UI.
* **Menu Sync Awareness:** The system should support future menu synchronization between the internal menu and aggregator platform menus. V1 focuses on order ingestion only.
